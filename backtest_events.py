...
# ------------ Data (yfinance only) ------------
def fetch_yf(symbol, years, interval="1wk", retries=3, sleep=3):
    err = None
...
def main():
    ...
    # Fetch from yfinance only
    try:
        df = fetch_yf(TICKER, years=cfg["rules"]["data"]["years"], interval=cfg["rules"]["data"]["interval"])
    except Exception as e:
    ...
    lines.append("")
    lines.append("Alignment breakdown (signal / direction / aligned):")
